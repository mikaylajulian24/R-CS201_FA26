...
author: "Amber Camp + Mikayla Julian"
date: "2026-08-31"
format: html
editor: visual
---
...
view(mtcars)
head(mtcars, 10)
# # number of rows to designate how many rows want to see
summary(mtcars) # opens help info tab
...
- 32 rows and 12 columns in the mtcars dataframe
...
mean(mtcars) #wrong
mean(mpg) #wrong again

mean(mtcars$mpg) # $ sign lets you select a variable/column within df 
...
# ggplot in tidyverse already 

ggplot(data = mtcars, aes(x = wt, y = mpg)) +
  geom_point(color = "pink") #gives scatter plot geometry
```
...
ggplot(data = mtcars, aes(x = cyl, y = disp)) +
  geom_point(color = "purple") #gives scatter plot geometry

#convert am to factor by creating new am_f variable
mtcars <- mtcars %>% 
  mutate(am_f = factor(am, labels = c("auto", "manual")))

ggplot(data = mtcars, aes(x = drat)) +
  geom_histogram(fill = "pink") #histogram geometry, don't put y bcs automatically puts                                  "count" as y 
