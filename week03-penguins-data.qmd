...
```{r, message=FALSE}
# command shift c to uncomment when highlighted 
...
# read csv function, and then path ()
...
# rename column name using snake_case
```
...
# updates every column to same case/lettering - snake_case
...
view(skim) # opens up skim tab to the right 
```
...
### Starting here on Friday 9/4/26
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.75) + #adds transparency with alpha command
  scale_fill_manual(values = c("darkorange", "darkorchid", "cyan4"), name = "Penguin Species") + #manual colors
  theme_bw() + #change background -> white now with bw but can choose 
  labs(title = "Palmer Penguinds' Flipper Length",
       subtitle = "Three species observed on three islands in the Palmer Archipelago",
       x = "Flipper on Length (mm)",
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.75) + #adds transparency with alpha command
  scale_fill_viridis_d(option  = "magma", name = "Penguin Species") + #manual colors
  theme_bw() + #change background -> white now with bw but can choose 
...
plotname <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, fill = species)) +
  geom_histogram() +
  theme_bw() +
  scale_fill_viridis_d(option  = "magma") +
  labs(title = "Palmer Penguins' Culmen Depth in mm",
       subtitle = "Penguin Species Culmen Depth in mm",
       x = "Culmen Depth (mm)",
       y = "Number of Penguins") 

ggplotly(plotname) #makes it interactive after assigning to plotname
...
ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  ...
  labs(title = "Palmer Penguins' Culmen Depth and Body Mass",
       subtitle = "Species observed",
       x = "Culmen Depth (mm)",
       y = "Body Mass (g)")
...
plot <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  theme_bw() +
  geom_point() +
  scale_color_viridis_d(option = "mako") +
  labs(title = "Palmer Penguins' Culmen Depth and Body Mass",
