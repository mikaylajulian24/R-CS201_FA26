...
Answer: distribution is skewed to the right - tail of unusually high values is pulling the mean up. median stays put because only cares about position, not magnitude.
...
ggplot(pokedata, aes(x = sp_atk)) +
  geom_histogram(binwidth = 5) # bindwidth is width of each bar
...
ggplot(pokedata, aes(x = sp_atk)) + 
  geom_histogram() +
  coord_cartesian(ylim = c(0, 25), xlim = c(150, 200))
...
Does that number match what you see in the scatterplot? yes. 0.5 correlation is more scattered
...
- How do you read a boxplot, and what are the different landmarks? middle lines, whiskers, dots, plots
